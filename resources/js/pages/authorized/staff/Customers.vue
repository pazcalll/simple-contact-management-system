...
const nextAjax = async () => {
  const response = await handleNextAJAX({ pagination: pagination, endpoint: '/staff/customers' });
  if (response == undefined) return;
  pagination.value = response as TPagination<TLead[]>;
};
const prevAjax = async () => {
  const response = await handlePrevAJAX({ pagination: pagination, endpoint: '/staff/customers' });
  if (response == undefined) return;
  pagination.value = response as TPagination<TLead[]>;
};
</script>

...
import { ROLE_STAFF } from '@/consts/role';
...
const staffs = page.props.staffs as TUser[];
...
if (page.props.flash.success) {
  ...
  submissionAlertState.value.message = page.props.flash.success;
}
...
const handleSubmitBulkAssign = () => {
  ...
  formBulkAssign.staff_id = selectedstaffId.value;
  formBulkAssign.is_unassign = formBulkAssign.is_unassign === 'on' ? 'on' : 'off';
  formBulkAssign.patch('/team-leaders/leads/bulk-assign-leads', {
    preserveState: false,
    onError: (err) => {
      console.log(err);
    },
...
const next = () => handleNext({ pagination: pagination, endpoint: '/team-leaders/leads' });
const prev = () => handlePrev({ pagination: pagination, endpoint: '/team-leaders/leads' });
</script>
        ...
        <BulkAssignDialog @submit="handleSubmitBulkAssign" v-model:open="isDialogOpen">
          ...
          <template #content>
            ...
            <div class="w-full">
              ...
              <ItemSelect placeholder="Select staff" :items="staffs" @update:selected-id="selectedstaffId = $event as number" />
              <p class="text-red-500" v-if="formAddNote.errors.note">{{ formAddNote.errors.note }}</p>
            </div>
            ...
            <TableRow v-for="lead in pagination.data" v-bind:key="lead?.id">
              ...
              <TableCell>
                <ul v-for="assignee in lead.users" :key="assignee.id" class="list-inside list-disc">
                  <li v-if="assignee.roles[0].name == ROLE_STAFF">{{ assignee.name }}</li>
                </ul>
              </TableCell>
            </TableRow>

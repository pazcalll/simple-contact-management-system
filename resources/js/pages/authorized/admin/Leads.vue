...
const handleSubmitBulkAssign = () => {
  ...
  form.submit('post', '/admins/leads/bulk-assign-leads', {
    ...
    onError: function (errors) {
      // 'errors' is provided by Inertia.js as a callback parameter, not imported.
...
<template>
  <AppLayout>
    <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
      ...
      <SubmissionAlert :isShow="submissionAlertState.isShow" :message="submissionAlertState.message" />
        ...
        <BulkAssignDialog @submit="handleSubmitBulkAssign" v-model:open="isDialogOpen">
          ...
          <template #content>
            <div class="w-full">
              ...
              <GroupedSelect
                :selectedId="selectedLeadStatusId"
                v-model="selectedLeadStatusId"
            ...
            <div class="w-full">
              <div class="flex justify-center space-x-1">
                ...
                <Label for="unassign"> Unassign </Label>
              </div>
            </div>
          </template>
        </BulkAssignDialog>
            ...
            <TableRow v-for="lead in pagination.data" v-bind:key="lead?.id">
              ...
              <TableCell>
                <GroupedSelect
                  ...
                  placeholder="Select lead status"
                  :convertable="leadStatuses"
                  @update:selected-id="(event) => handleLeadStatusChanged(lead.id, event)"
                />
